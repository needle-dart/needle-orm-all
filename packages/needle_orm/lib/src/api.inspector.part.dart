...
abstract class ModelInspector<M extends Model> {
  ...
  static _ModelHelper _helper(Model model) {
    return model._innerHelper;
  }

  static void registerClass(String className, OrmMetaClass metaClass) {
    metaMap[className] = metaClass;
  }

  static void registerAllClass(List<OrmMetaClass> metaClasses) {
 ...
 */
    ...
    Model model = inspector.newInstance(topQuery: topQuery);

    if (id != null) {
      inspector.setFieldValue(
          model, ModelInspector.idFields(className)!.first.name, id);
    }
