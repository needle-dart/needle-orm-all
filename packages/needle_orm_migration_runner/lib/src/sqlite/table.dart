...
abstract class SqliteGenerator {
  ...
  static String compileColumn(MigrationColumn column) {
    var buf = column.indexType == IndexType.primaryKey
        ? StringBuffer('INTEGER')
        : StringBuffer(columnType(column));
    ...
    if (column.indexType == IndexType.unique) {
      ...
      buf.write(' PRIMARY KEY AUTOINCREMENT');
    }
...
class SqliteTable extends MigrationTable {
  ...
  void compile(StringBuffer buf, int indent) {
    ...
    _columns.forEach((name, column) {
      var col = SqliteGenerator.compileColumn(column);
      if (i++ > 0) buf.writeln(',');
...
class SqliteAlterTable extends MigrationTable implements MutableTable {
  final Map<String, MigrationColumn> _columns = {};
  final String tableName;
  final Queue<String> _stack = Queue<String>();

  SqliteAlterTable(this.tableName);
  ...
  void changeColumnType(String name, ColumnType type, {int length = 256}) {
    _stack.add(
        'ALTER COLUMN $name TYPE ${SqliteGenerator.columnType(MigrationColumn(type, length: length))}');
  }
